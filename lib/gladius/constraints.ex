defmodule Gladius.Constraints do
  ...
  defp check_one(_value, {:filled?, false}), do: []
  ...
  defp check_max_length(v, n) when is_binary(v) do
    if byte_size(v) <= n,
      do: [],
      else: [err(:max_length, v, "must be at most #{n} characters", [max: n])]
  end
  defp check_max_length(v, n) when is_list(v) do
    if length(v) <= n,
      do: [],
      else: [err(:max_length, v, "must have at most #{n} items", [max: n])]
  end
  ...
  defp check_size(v, n) when is_binary(v) do
    if byte_size(v) == n,
      do: [],
      else: [err(:size?, v, "must be exactly #{n} characters", [size: n])]
  end
  defp check_size(v, n) when is_list(v) do
    if length(v) == n,
      do: [],
      else: [err(:size?, v, "must have exactly #{n} items", [size: n])]
  end
  ...
  defp check_format(v, %Regex{} = r) when is_binary(v) do
    if Regex.match?(r, v),
      do: [],
      else: [err(:format, v, "must match format #{inspect(r)}", [format: r])]
  end
  ...
  defp check_in(v, values) when is_list(values) do
    if v in values,
      do: [],
      else: [err(:in?, v, "must be one of #{inspect(values)}", [values: values])]
  end

  # ---------------------------------------------------------------------------

  defp err(pred, value, default_message, bindings) do
    message = translate(default_message, pred, bindings)
    %Gladius.Error{
      predicate:        pred,
      value:            value,
      message:          message,
  ...
  defp translate(default_message, _pred, bindings) do
